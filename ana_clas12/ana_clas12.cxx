...
Bool_t ana_clas12::Process(Long64_t entry)
{
   ...
   // It can be passed to either ana_clas12::GetEntry() or TBranch::GetEntry()
  ...
  for(int j=0;j<size;j++)
    {
      ...
      //electron ID
      if((REC_Particle_charge)[j]<0 && (REC_Particle_pid)[j]==11 && (REC_Particle_status)[j]>1000)
	{
	  nelec++;
	  i_el=j;
      ...
      if(nelec>=1)
	{
	  ...
	  if(j!=i_el && j!=i_ph && (REC_Particle_pid)[j]==2212)
	    {
	      //proton ID
	      //nprot++;
	      i_pr=j;
	  ...
	  if(j!=i_el && j!=i_ph && j!=i_pr && (REC_Particle_charge)[j]==0 && (REC_Particle_pid)[j]==2112)
	    {
        //nneut++;
	      i_n=j;
	      ...
	      else if((REC_Particle_status)[j]>=4000)CD_neutron=1;
